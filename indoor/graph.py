	...
	with tf.variable_scope("geometric"):
		...
		for l in range(opt.warpN):
			with tf.variable_scope("warp{0}".format(l)):
				...
				feat = imageConcat # 120x160
				with tf.variable_scope("conv1"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,32) # 60x80
				with tf.variable_scope("conv2"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,64) # 30x40
				with tf.variable_scope("conv3"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,128) # 15x20
				with tf.variable_scope("conv4"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,256) # 8x10
				with tf.variable_scope("conv5"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,512) # 4x5
...
def discriminator(opt,image,reuse=False):
	...
	with tf.variable_scope("discrim",reuse=reuse):
		feat = image # 120x160
		with tf.variable_scope("conv1"): feat = conv2Layer(opt,feat,32) # 60x80
		with tf.variable_scope("conv2"): feat = conv2Layer(opt,feat,64) # 30x40
		with tf.variable_scope("conv3"): feat = conv2Layer(opt,feat,128) # 15x20
		with tf.variable_scope("conv4"): feat = conv2Layer(opt,feat,256) # 8x10
		with tf.variable_scope("conv5"): feat = conv2Layer(opt,feat,512) # 4x5
		with tf.variable_scope("conv6"): feat = conv2LayerScore(opt,feat,1) # 4x5
		score = feat
	return score
